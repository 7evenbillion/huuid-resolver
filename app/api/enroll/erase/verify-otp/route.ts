...
export async function POST(req: NextRequest) {
  ...
  if (!session) {
    return NextResponse.json({ error: 'Your session has expired. Please start again.' }, { status: 400 });
  }
  ...
  const { data: otpRowData, error: findError } = await client
    .rpc('huuid_otp_find_active', { p_phone: session.phone, p_otp_type: 'erasure', p_pii_key: piiKey })
    .maybeSingle();
  const otpRow = otpRowData as OtpRow | null;

  if (findError) {
    console.error(JSON.stringify({ level: 'error', action: 'erasure_verify_otp_find_failed', message: findError.message }));
  ...
  const attempts = otpRow.attempts;

  if (attempts >= OTP_MAX_ATTEMPTS) {
    const lockoutEnd = createdAt + OTP_LOCKOUT_MINUTES * 60 * 1000;
    if (now < lockoutEnd) {
      const remainingMinutes = Math.ceil((lockoutEnd - now) / 60000);
      return NextResponse.json({ error: `Too many attempts. Please wait ${remainingMinutes} minute(s).`, code: 'locked' }, { status: 429 });
    }
    return NextResponse.json({ error: 'This code has expired. Request a new one.', code: 'expired' }, { status: 400 });
  }
  if (now > expiresAt) {
  ...
  if (!safeEqualHex(enteredHash, otpRow.otp_hash)) {
    ...
    return NextResponse.json({ error: `Incorrect code. ${remaining} attempt(s) remaining.`, code: 'incorrect', attemptsRemaining: remaining }, { status: 400 });
  }

  await client.rpc('huuid_otp_mark_used', { p_id: otpRow.id });
  await eraseSession.update({ phoneVerified: true });
  await writeEnrollmentAudit({ huuid: null, action: 'phone_verified', ipHash, userAgentHash: uaHash, outcome: 'erasure_success' });
