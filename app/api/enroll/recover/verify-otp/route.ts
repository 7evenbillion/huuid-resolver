...
export async function POST(req: NextRequest) {
  ...
  if (!session) {
    return NextResponse.json({ error: 'Your recovery session has expired. Please start again.' }, { status: 400 });
  }
  ...
  const { data: otpRowData, error: findError } = await client
    .rpc('huuid_otp_find_active', { p_phone: session.phone, p_otp_type: 'recovery', p_pii_key: piiKey })
    .maybeSingle();
  const otpRow = otpRowData as OtpRow | null;

  if (findError) {
    console.error(JSON.stringify({ level: 'error', action: 'recovery_verify_otp_find_failed', message: findError.message }));
  ...
  await recoverySession.update({ phoneVerified: true });
  await writeEnrollmentAudit({ huuid: null, action: 'phone_verified', ipHash, userAgentHash: uaHash, outcome: 'recovery_success' });
