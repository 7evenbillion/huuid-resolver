...
export async function POST(req: NextRequest) {
  const session = await myHuuidOtpLoginSession.get();
  if (!session) {
  ...
  const { data: otpRowData, error: findError } = await client
    .rpc('huuid_otp_find_active', { p_phone: session.phone, p_otp_type: 'login', p_pii_key: piiKey })
    .maybeSingle();
  const otpRow = otpRowData as OtpRow | null;

  if (findError) {
    console.error(JSON.stringify({ level: 'error', action: 'my_huuid_otp_login_verify_find_failed', message: findError.message }));
  ...
  await patientSession.set({ huuid: session.huuid, phone: session.phone, phoneVerified: true, createdAt: Date.now() });
  await myHuuidOtpLoginSession.clear();
